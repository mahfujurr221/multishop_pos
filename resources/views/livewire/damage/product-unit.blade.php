<div>
    <div class="form-group">
      ...
      <div class="form-group">
        ...
        <select name="product_variation_id" class="form-control" data-provide="selectpicker" data-live-search="true" data-size="10" wire:model="selected_variation">
          <option value="">Select Variation</option>
          @foreach ($product->variations as $variation)
            <option value="{{ $variation->id }}" {{ old("product_id")==$variation->id?"SELECTED":"" }}>{{ $variation->name }}</option>
          @endforeach
        </select>
        @if($errors->has('product_variation_id'))
          <div class="alert alert-danger">{{ $errors->first('product_variation_id') }}</div>
        @endif
      </div>
      {{-- @dump($selected_size) --}}
