<script>
    ...
    function domPrepend(product = null, index = null) {
        ...
        var type=$('.price_group').find(':selected').text();
        // alert(type);
        let price=0;
        if(type=='Fixed Price'){
            price = product.price;
    ...
    function totalCalculate() {
        ...
        let costsubTotalList = document.querySelectorAll('.cost_sub_total');
        ...
        $.each(costsubTotalList, (index, value) => {
            cost_total += parseFloat(value.value);
        });

        $("#cost_input").val(cost_total);
    ...
    function calculate_total_receivable() {
        ...
        deliveryCharge = empty_field_check(deliveryCharge);
        ...
        $("#after_delivery_cost").text(totalAmount + parseFloat(deliveryCharge));
        $("#receivable_input").val($("#receivable").text() - discountAmount);
        $("#receivable_input").val(totalAmount + parseFloat(deliveryCharge));
    ...
    $("#delivery_charge").on('keyup', function() {
        calculate_total_receivable();
    });
    ...
    $("#paid_btn").on('click', function() {
        var costing = empty_field_check($("#after_delivery_cost").text());
