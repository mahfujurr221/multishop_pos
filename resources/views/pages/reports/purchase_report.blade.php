    ...
    <div class="card card-body mb-2">
            <form action="{{ route('report.purchase_report') }}">
                ...
                data-live-search="true" data-size="10">
                        ...
                        <option value="{{ $item->id }}" {{ isset($product_id)&&$product_id==$item->id?"SELECTED":"" }}>{{ $item->name }} - {{ $item->code }}</option>
                        @endforeach
                  </select>
                    </div>
                </div>
            ...
            <div class="table-responsive table-responsive-sm">
                <table class="table table-bordered" data-provide="">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td scope="row">{{ $purchases->currentPage()!=1?$purchases->currentPage()*$purchases->perPage()+$key+1:++$key }}</td>
                            <td>
