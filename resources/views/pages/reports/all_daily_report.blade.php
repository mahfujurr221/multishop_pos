...
@section('title', 'All Shop Daily Report')

@section('page-header')
<header class="header bg-ui-general">
     <div class="header-info">
          <h1 class="header-title">
               <strong>All Shop Daily Report</strong>
          </h1>
     </div>
</header>
@endsection
		...
		<table class="table table-striped table-bordered">
			...
			<tbody>
				...
				@foreach($daterange as $date)
					...
					$sell_and_profit=$summary::sell_profit($date,$date,true);
					//dd($sell_and_profit);
					@endphp
					<tr>
						<td>{{$date}}</td>
						<td>{{ $sold=$sell_and_profit['sell_value'],$total_sold+=$sold }}</td>
						<td>{{ $purchased=$summary::date_purchased($date,true),$total_purchased+=$purchased }}</td>
						<td>{{ $spent=$summary::date_expense($date,true),$total_spent+=$spent }}</td>
						<td>{{ $returned=$summary::returned($date,$date,true),$total_returned+=$returned }}</td>
						<td>{{ $gross_profit=$sell_and_profit['profit'],$total_gross+=$gross_profit }}</td>
						@php
							$discount=$summary::date_discount($date,true);
						@endphp
