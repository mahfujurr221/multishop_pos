...
class TransactionService
{
    ...
    {
        $last_transaction = $from_transaction;
        $transactions = Transaction::where('id', '>', $from_transaction->id)->where('customer_id', $from_transaction->customer_id)->get();
        foreach ($transactions as $transaction) {
            if ($transaction->debit) {
                $transaction->update([
                    'balance' => $last_transaction->balance + $transaction->debit
                ]);
            } elseif ($transaction->credit) {
                $transaction->update([
                    'balance' => $last_transaction->balance - $transaction->credit
                ]);
            }
    ...
    {
        $last_transaction = $from_transaction;
        $transactions = Transaction::where('id', '>', $from_transaction->id)->where('supplier_id', $from_transaction->supplier_id)->get();
        foreach ($transactions as $transaction) {
            if ($transaction->debit) {
                $transaction->update([
                    'balance' => $last_transaction->balance + $transaction->debit
    ...
    public static function customer_payment_transaction($actual_payment)
    {
        ...
        if ($actual_payment->payment_type == 'receive') {
            $actual_payment->transaction()->create([
                ...
                'particulars' => 'Received from Customer',
                'customer_id' => $actual_payment->customer_id,
                'balance' => $balance - $actual_payment->amount
            ]);
    ...
    public static function create_payment_transaction($actual_payment_id)
    {
        ...
        // info($actual_payment);
        if ($actual_payment&&$actual_payment->supplier_id) {
            static::supplier_payment_transaction($actual_payment);
        } else if ($actual_payment&&($actual_payment->customer_id != null || $actual_payment->customer_id == 0)) {
