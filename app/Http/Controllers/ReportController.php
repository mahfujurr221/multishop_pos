...
class ReportController extends Controller
{
    ...
    public function summary_report(Request $request)
    {
        ...
        return view('pages.reports.summary_report', compact('sells', 'expenses', 'pos_items', 'payments_paid', 'payments_received','start_date','end_date'));
    }
    ...
    public function all_daily_report(Request $request)
    {
        $start_date = date('Y-m-1');
        $end_date = date('Y-m-t');
		...
		return view('pages.reports.all_daily_report',compact('start_date','end_date'));
    }

    public function seller_report(){
        $users=User::paginate(10);
        return view('pages.reports.seller_report',compact('users'));
    }
    ...
    public function purchase_report(Request $request)
    {
        ...
        $purchases = new PurchaseItem();
        if ($request->product_id) {
            $purchases = $purchases->where('product_id', $request->product_id);
        }

        $purchases = $purchases->where('transfer_product_id',NULL)->paginate(20);
